...
def generate(model, tok, samples, max_new_tokens=512):
    outs = []
    for s in samples:
        msgs = [{"role": "system", "content": s["instruction"]},
                {"role": "user", "content": s.get("input") or "（继续对话）"}]
    ...
    for t in outputs:
        ...
        if all(k in obj for k in REQUIRED_KEYS):
            ...
            if DESC_MIN <= len(obj.get("description", "")) <= DESC_MAX:
                len_ok += 1
    return {"JSON可解析率": parse_ok / n, "必填字段完整率": field_ok / n, "长度合规率": len_ok / n}
